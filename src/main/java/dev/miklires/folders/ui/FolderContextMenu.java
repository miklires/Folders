...
/**
 * The compact right-click menu from §37.
 *
...
public final class FolderContextMenu {
    ...
    public record Item(Text label, Runnable action, boolean enabled) {
        public static Item of(String key, Runnable action) {
            return new Item(Text.translatable(key), action, true);
        }

        public static Item of(String key, Runnable action, boolean enabled) {
            return new Item(Text.translatable(key), action, enabled);
        }
    }
    ...
    public static FolderContextMenu open(List<Item> items, int mouseX, int mouseY, int screenWidth, int screenHeight) {
        ...
        int width = MIN_WIDTH;
        for (Item item : copy) {
            width = Math.max(width, GuiCompat.textWidth(item.label()) + PADDING * 2 + 4);
        }
    ...
    public void render(DrawContext context, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (item.enabled() && isOverRow(mouseX, mouseY, rowY)) {
                GuiCompat.fill(context, x + 1, rowY - 1, x + width - 1, rowY + ROW_HEIGHT - 2, HOVER);
            }
            GuiCompat.drawTrimmedText(context, item.label(), x + PADDING, rowY, width - PADDING * 2,
