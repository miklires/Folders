...
/**
 * The "Default / Choose PNG / Reset" flow
 *
    ...
    private static Path promptForPng() {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            String chosen = TinyFileDialogs.tinyfd_openFileDialog(
                    net.minecraft.network.chat.Component.translatable("folders.icon.choose").getString(),
                    null, filters, "PNG (*.png)", false);
            return chosen == null ? null : Path.of(chosen);
        } catch (Throwable error) {
            // A missing or refused native dialog must not break the screen.
